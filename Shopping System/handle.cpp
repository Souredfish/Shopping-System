...
void Handle::H_createsell()
{
    ...
    int random_id1=0, random_id2=0;//保存两个商品的id
...
void Handle::H_report() {
    int max = 0;
    int i = 0;
    int id = 0;
    for (i = 0; i < log->ReSize(); i++) {
        if (log->nums[i] > max) {
            id = i;
            max = log->nums[i];
        }
    ...
    std::string maxstr;
    maxstr = "H:[Report]The best seller in the past quarter:" + std::to_string(id+1) + "(id)"+ "  num:"+std::to_string(max);
    h_msg->msg_add(maxstr, false);
}
    ...
    for (int i = 0; i < idnum; i++) {
        ...
        for (int j = 0; j < nowid; j++) {
            ...
            if (year > dyear) {
                ...
                string str = "H:[warn]Some commondity (id)" + to_string(ptr->WhC_REID()) + "overdue!";
                h_msg->msg_add(str, false);
            }
            else {
                //没有过期商品 无需操作
